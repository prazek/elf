...
std::tuple<elf_symbol_table, elf_symbol_strings> read_symbol_table(const Elf32_Shdr& section_header,
  ...
  std::vector<Elf32_Sym> symbol_table = [&] {
    ...
    const auto* begin = reinterpret_cast<const Elf32_Sym*>(symbol_table_bytes.get());
...
elf_rel_table read_rel_table(const Elf32_Shdr& section_header, std::fstream& file) {
  ...
  const auto* begin = reinterpret_cast<const Elf32_Rel*>(rel_table_bytes.get());
  const auto* end = begin + symbol_count;

  return elf_rel_table(begin, end);
}

std::vector<Elf32_Phdr> read_program_headers(const Elf32_Ehdr& header, std::fstream& elf_file) {
  ...
  for (int i = 0; i < header.e_phnum; i++) {
    if (!elf_file.read(reinterpret_cast<char*>(&program_headers[i]), header.e_phentsize))
