...
int set_register_64(unsigned arg_num, int stack_offset, char* code) {
  if (arg_num < NUM_PASSING_REGISTERS) {
    ...
    std::memcpy(code, MOVE_64_TO_REGISTER[NUM_PASSING_REGISTERS], sizeof(MOVE_64_TO_REGISTER[0]));
  }
...
int set_register_32(unsigned arg_num, int stack_offset, char* code) {
  if (arg_num < NUM_PASSING_REGISTERS) {
    ...
    std::memcpy(code, MOVE_32_TO_REGISTER[NUM_PASSING_REGISTERS], sizeof(MOVE_32_TO_REGISTER[0]));
  }
  assert(stack_offset < 127);
  auto offset = static_cast<char>(stack_offset);
...
int set_register_signed_extend_32(unsigned arg_num, int stack_offset, char* code) {
  if (arg_num < NUM_PASSING_REGISTERS) {
    std::memcpy(code, MOVE_32_SIGNED_TO_REGISTER[arg_num], sizeof(MOVE_32_SIGNED_TO_REGISTER[0]));
  } else {  // Put into rax
    std::memcpy(code, MOVE_32_SIGNED_TO_REGISTER[NUM_PASSING_REGISTERS], sizeof(MOVE_32_SIGNED_TO_REGISTER[0]));
  }
  assert(stack_offset < 127);
  auto offset = static_cast<char>(stack_offset);
...
int set_register(unsigned arg_num, type arg_type, int stack_offset, char* code) {
  switch (arg_type) {
...
void set_returned_value(type return_type, char*& code) {
  ...
  if (return_type == TYPE_LONG) {
    std::memcpy(code, CHECK_RETURNED_SIGNED_LONG, sizeof(CHECK_RETURNED_SIGNED_LONG));
  ...
  if (return_type == TYPE_UNSIGNED_LONG || return_type == TYPE_PTR) {
    std::memcpy(code, CHECK_RETURNED_UNSIGNED_LONG_OR_PTR, sizeof(CHECK_RETURNED_UNSIGNED_LONG_OR_PTR));
...
jmp_buf& get_exit_jump_buffer() { return exit_buff; }
...
void* create_trampoline(const function& func, char*& code, char* code_end) {
  ...
  assert(code_end - code > MAX_TRAMPOLINE_SIZE && "Need to have enough space for trampoline");

  std::memcpy(code, TRAMPOLINE_SWITCH_TO_64_BITS, sizeof(TRAMPOLINE_SWITCH_TO_64_BITS));
  ...
  std::memcpy(code + RETURN_TO_32_BITS_ADDR_INDX, &addr_after_returning_to_32_bit, 4);
...
std::unordered_map<std::string, void*> create_trampolines(const function* funcs, const int nfuncs,
  ...
  auto alloc_trampolines_code = [] {
    char* memory = (char*)mmap(NULL, TRAMPOLINES_BUFFER_SIZE, PROT_EXEC | PROT_READ | PROT_WRITE,
  ...
  struct function exit_func = {"exit", exit_types, 1, TYPE_VOID, (void*)exit_64bit};
  ...
  for (auto& trampoline_buff : alloced_trampolines) {
    if (mprotect(trampoline_buff.get(), TRAMPOLINES_BUFFER_SIZE, PROT_EXEC | PROT_READ) != 0)
